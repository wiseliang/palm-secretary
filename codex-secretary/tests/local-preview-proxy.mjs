...
const server = http.createServer((request, response) => {
  ...
  const headers = targetPort === apiPort ? { ...request.headers, 'tailscale-user-login': 'test@example.com' } : request.headers;
  const upstream = http.request({ hostname: '127.0.0.1', port: targetPort, path: request.url, method: request.method, headers }, (incoming) => {
...
server.on('upgrade', (request, socket, head) => {
  const upstream = net.connect(apiPort, '127.0.0.1', () => {
    const headers = Object.entries({ ...request.headers, 'tailscale-user-login': 'test@example.com' }).map(([name, value]) => `${name}: ${Array.isArray(value) ? value.join(', ') : value}`).join('\r\n');

...
const store = await readFile(new URL('../server/project-store.ts', import.meta.url), 'utf8');
...
const harmony = await readFile(new URL('../../palm-secretary-harmony/entry/src/main/ets/pages/Index.ets', import.meta.url), 'utf8');

for (const route of ['upload-session', 'upload-chunk', 'upload-complete']) assert.match(server, new RegExp(`/api/files/${route}`));
